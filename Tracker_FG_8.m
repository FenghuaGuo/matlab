 classdef Tracker_FG_8 < handle
    ...
    methods (Access = public)
       ...
       function this = setAFD(this, afd)
        ...
        function [tract, tractVal,tracte,tractFOD] = track(this, point) %tractDir,averageDir,stopVal,stopAngle
            point = single(point);
...
% % %             val = [val ab];
            ...
            [tract1, tractVal1,tractCSD_FOD1,tracte1] = this.trackOneDir(point, dir); %add voxel_th, tractDir1,averageDir1,
            if this.pb; progressbar('ready'); end

            if this.pb; progressbar('start', [size(point,2) 1], 'Tracking in second direction'); end
            [tract2, tractVal2,tractCSD_FOD2,tracte2] = this.trackOneDir(point, -dir); %add voxel_th, tractDir2,averageDir2,
            if this.pb; progressbar('ready'); end
...
%             tracte = cell(size(tracte2));
    ...
    methods (Access = private)
        function [tract, tractVal,tractCSD_FOD,tracte] = trackOneDir(this, point, dir) %tractDir,averageDir,
...
%                     averageDir(:,i) = nanmean(tractDir{flist(i)}); % this can be used earlier as an additional angular threshold
        ...
        function point_n6 = Get_neighbour (this,point)
                ...
                temp = point;
                temp(1,:) = point(1,:)-1;
                point_n6{1,1} = temp; classdef Tracker_FG_8 < handle
                    %
                    % Abstract Tractography class
                    %
                    % see also DTITracker, FODTracker
